...
$RealValidReport = "$userProfile\Documents\Reports\DBS\DBS Valid Report after $(get-date -f dd-MM-yyy).csv"
#-----------FAIL REPORTS
...
$RealExpiredReport = "$userProfile\Documents\Reports\DBS\DBS Expired Report before $(get-date -f dd-MM-yyy).csv"
#-------------------------------------------------------
...
$myCsvFile =Import-CSV "$dataPath" -Header "Surname", "FirstName", "Reference", "Unit", "JoiningDate", "Position", "CheckType", "DateDisclousure", "RenewalDate", "Latest", "BackgroundCheck" | 
 Where-Object { $_."BackgroundCheck" -eq "DBS Update Service Subscription Paid" -and $_."BackgroundCheck" -ne "DBS Update Service Check" -and ($_."RenewalDate" -as [DateTime]) -gt $expiredDateSubscription}
 } #-close Try
...
Try{
$myCsvFile =Import-CSV "$dataPath" -Header "Surname", "FirstName", "Reference", "Unit", "JoiningDate", "Position", "CheckType", "DateDisclousure", "RenewalDate", "Latest", "BackgroundCheck" | 
 Where-Object { $_."BackgroundCheck" -eq "DBS Check" -and $_."BackgroundCheck" -ne "DBS Update Service Check" -and ($_."RenewalDate" -as [DateTime]) -gt $expiredDate}
...
Catch{Write-Warning "MainDBS.csv file not found on $userProfile`\Downloads"}
    ...
    ForEach($object in $myCsvFile){
            ...
            if($Reference -in $validUpdate."Reference" -and $BackgroundCheck -eq "DBS Check"){
            $arrayValid += $row} # get valid
            if($Reference -in $validCheck."Reference" -and $BackgroundCheck -eq "DBS Check" -and $Reference -notin $validUpdate."Reference" -and $Reference -notin $expiredUpdate."Reference") {
            $arrayValid += $row} # get valid
